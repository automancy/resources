...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let edge_effect = depth_edge(in.uv) * normal_edge(in.uv);

    let screen_effect_enabled = (ubo.flags.x & FLAG_SCREEN_EFFECT) != 0u;
    if screen_effect_enabled {
        let edge_darken = smoothstep(0.0, 1.0, pow(1.0 - distance, 1.5));
        let chroma_abbr = vec3(
        ...
        return vec4(((color.rgb + chroma_abbr) * 0.5) * edge_darken * edge_effect, color.a);
    } else {
        return vec4(color.rgb * edge_effect, color.a);
    }
}
