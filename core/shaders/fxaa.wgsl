...
const FXAA_SPAN_MAX: f32 = 3.0;
const FXAA_REDUCE_MIN: f32 = 0.00015;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let rgba_a = (1.0 / 2.0) * (textureSample(frame_texture, frame_sampler, in.uv + texel_dir * (1.0 / 3.0 - 0.5)) + textureSample(frame_texture, frame_sampler, in.uv + texel_dir * (2.0 / 3.0 - 0.5)));

    let rgba_b = rgba_a * (1.0 / 2.0) + (1.0 / 4.0) * (textureSample(frame_texture, frame_sampler, in.uv + texel_dir * (0.0 / 3.0 - 0.5)) + textureSample(frame_texture, frame_sampler, in.uv + texel_dir * (3.0 / 3.0 - 0.5)));
